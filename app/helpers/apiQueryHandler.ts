...
export function useForexApi() {
  ...
  const formatHistoricalAPIQuery = ({ queryCode, fromSymbol, toSymbol }: HistoricalAPIQueryTypes) => {
  ...
  const formatExchangeAPIQuery = ({ fromSymbol, toSymbol }: ExchangeAPIQueryTypes) => {
  ...
  const getHistoricalData = async ({ queryCode, fromSymbol, toSymbol }: HistoricalAPIQueryTypes) => {
    const requestURL = formatHistoricalAPIQuery({ queryCode, fromSymbol, toSymbol })
    ...
    try {
      ...
      if (data["Information"]) throw new Error("Exceeded 25/Day Limit")
      ...
      const parsedTimeseries = Object.entries(timeseries) as [string, OHLCTypes][]
      const timeseriesData = parsedTimeseries.slice(0,100)
      timeseriesData.reverse()

      return {
        timeseriesLabels: timeseriesData.map(([time]) => time),
        timeseriesData: timeseriesData.map(([_, ohlc]) => Number(ohlc["4. close"]))
      }
    } catch (err) {
      console.error(err)
      return err || "Failed to fetch historical data"
    }
  }

  const getExchangeRate = async ({ fromSymbol, toSymbol }: ExchangeAPIQueryTypes) => {
  ...
  const getHistoricalDataDemo = async (type: string) => {
    ...
    try {
      ...
      console.error(err)
      return err || "Failed to fetch demo historical data"
    }
  }
  ...
  const getExchangeRateDemo = async () => {
    try {
      ...
      console.error(err)
      return err || "Failed to fetch demo exchange rate"
    }
  }
